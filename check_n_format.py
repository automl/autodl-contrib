...
def compute_stats(labels_df, label_name=None):
    ...
    if label_name is not None:
        if (label_name.shape[0] != res['label_num']):
            raise Exception(
                'Number of labels found in label.name and computed manually is not the same: {} != {}'
                .format(label_name.shape[0], res['label_num']))
    res['domain'] = 'image'
    return res
...
def find_file(input_dir, name):
    ...
    filename = [
        file
        for file in glob.glob(os.path.join(input_dir, '*{}*'.format(name)))
    ]
    return filename[0]
...
def manual_check(data_dir, num_examples=5):
    ...
    print(
        'Samples of the dataset are going to be displayed. Please check that the display is correct. Click on the cross after looking at the images.'
    )
...
def format_automatically(input_dir, train_size, num_channels):
    ...
    print('\nDataset fake name: {}\n'.format(fake_name))
    labels_df = format_image.get_labels_df(
        input_dir)  # same function in format_video
    ...
    effective_sample_num = res[
        'sample_num']  # if quick check, it'll be the number of examples to format for each class

    if not is_formatted(output_dir):
        print('No formatted version found, creating {} folder.'.format(
            output_dir))
        os.mkdir(output_dir)
    ...
    format_data(input_dir,
                ...
                train_size=train_size,
                num_channels=num_channels,
                classes_list=flat_label_list,
                domain=domain)
    formatted_dataset_path = os.path.join(output_dir, fake_name)

    # run baseline
    if do_run_baseline:
        code_dir = os.path.join(STARTING_KIT_DIR,
...
if __name__ == "__main__":
    ...
    if len(argv) == 2:
        ...
        print(
            'Please enter a dataset directory. Usage: `python3 check_n_format path/to/dataset`'
        )
        exit()
    ...
    if not input('Quick check? [Y/n] ') in ['n', 'N']:
        # quick check
        print(
            'Quick check enabled: running script on a small subset of data to check if everything works as it should.'
        )
    ...
    if is_formatted(output_dir):
        # Already exists
        if not input('Overwrite existing formatted data? [Y/n] ') in [
                'n', 'N'
        ]:
            # Overwrite
            if input('Re-format all {} files? [Y/n] '.format(
                    effective_sample_num * quick_check)) in ['n', 'N'
                                                             ]:  # Confirmation
                # Do nothing
                exit()
        else:
    ...
    # booleans
    do_run_baseline = not input('Run baseline on formatted data? [Y/n] ') in [
        'n', 'N'
    ]
